...
function diagnostic(name: string, wall: number | null): DiagnosticSummary {
  return {
    ...base,
    name,
...
describe("resourceUsageCsv", () => {
  it("orders by total wall time with untimed rows last", () => {
    ...
    expect(lines[1]).toBe(
      'PMP,"Fast, quick","fast, quick",2,100,50,100,,,1024',
    );
    expect(lines[2]).toBe("PMP,Slow,slow,2,10,5,10,,,1024");
