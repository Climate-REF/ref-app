...
export function SeriesVisualization({
  ...
  // Common function to get hash-based index from dimension value
  const getDimensionHashIndex = useCallback(
    (seriesKey: string, dimension: string, arrayLength: number): number => {
  ...
  const getSeriesColor = useCallback(
    ...
    [hueDimension, getDimensionHashIndex],
  );

  // Memoize line style calculations
  const getSeriesStyle = useCallback(
    (seriesKey: string, index: number) => {
      if (styleDimension && styleDimension !== "none") {
        return LINE_STYLES[
    ...
    [styleDimension, getDimensionHashIndex],
  );
                      ...
                      {seriesKeys
                        .sort((a, b) => {
                          ...
                          const getPriority = (
                            ...
                            if (isReference && !isHidden) return 3; // Reference lines second highest
                            if (!isHidden) return 2; // Regular visible lines
